...
def main():
    ...
    data_poth = f"{source_path}_unfiltered" if args.unfiltered else source_path

    print(f"data path: {data_poth}")

    features = np.load(f"{data_poth}.npy", mmap_mode="r")
    pca_A = torch.from_numpy(np.load(f"{args.pca_path}_A.npy")).cuda()
    pca_b = torch.from_numpy(np.load(f"{args.pca_path}_b.npy")).cuda()

    os.makedirs(args.save_dir, exist_ok=True)
    save_path = osp.join(args.save_dir, args.split)

    copyfile(f"{source_path}.tsv", f"{save_path}.tsv")
    copyfile(f"{data_poth}.lengths", f"{save_path}.lengths")

    if osp.exists(f"{source_path}.phn"):
        copyfile(f"{source_path}.phn", f"{save_path}.phn")

    if osp.exists(f"{source_path}.wrd"):
